...
```{r}
x.a <- rnorm(1000, mean = 1, sd = 10)

# 1.1 Filter OUT extreme values out of two standard deviations
x.a.remain <- x.a[x.a > mean(x.a) - 2 * sd(x.a) & x.a < mean(x.a) + 2 * sd(x.a)]

# 1.2 Plotting the distribution of the remaining vector x.a
plot(density(x.a.remain), main = "PDF of rnorm(1000, 1, 10) w/i 2std")

# 1.3 Calculate the 25% 50% and 75% quantile of vector x.a. (You may google "quantile r)
quant <- quantile(x.a); quant
print(paste("The 25%, 50% and 75% quantile of vector x.a. are", round(quant[2], 2),",", round(quant[3], 2),"and", round(quant[4], 2), ", respectively."))

# 1.4 Get the sequence between 25% to 75% of x.a
x.a[x.a > quant[2] & x.a < quant[4]]
```
...
dt3.3 <- data.frame(population, town)
as.character(arrange(dt3.3, desc(population))$town)
```
...
* `print()`:  Each output is preceded by `[x]` followed by quotations around the actual content. The `x` in this case refers to the element number being printed.   
* `cat()`: Compared to `print()`, each output of `cat()` is plain text, without `[x]` and quotations.    
* `message()`: Like `cat()` but this time color the output with some other colors in your R console.   
* Ref: [https://reurl.cc/al9GQZ](https://reurl.cc/al9GQZ)   
...
* `order(a, b)`: Sort the data.frame in orders of increasing `b`, and then increasing `a`.
* The negative sign represents the decreasing order.
...
dt1.3.2 <- data.frame(country, import, export)
...
# Reorder
dt1.3.2 <- dt1.3.2[order(-dt1.3.2$import, -dt1.3.2$export),] #先排出口再入口
dt1.3.2 %>%
  kable() %>%
  kable_styling()
```
